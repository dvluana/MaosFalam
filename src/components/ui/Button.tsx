...
const base =
  "font-raleway uppercase tracking-[0.06em] inline-flex items-center justify-center gap-2 transition-all duration-300 branded-radius corner-ornaments disabled:opacity-20 disabled:pointer-events-none cursor-pointer";
...
const variantClass: Record<Variant, string> = {
  ...
  ghost:
    "bg-transparent border-0 text-bone-dim hover:text-gold shadow-none",
  icon: "",
};
...
export default function Button({
  ...
  if (variant === "icon") {
    return (
      <button
        {...rest}
        className={`w-11 h-11 flex items-center justify-center bg-transparent text-bone border border-[rgba(201,162,74,0.08)] branded-radius corner-ornaments transition-all hover:bg-[rgba(123,107,165,0.06)] ${className}`}
      >
        {children}
      </button>
    );
  ...
  const primaryStyle =
    variant === "primary"
      ? {
          background:
            "linear-gradient(165deg, #2c2250 0%, #3a2a68 45%, #2a1f4a 100%)",
        }
      : undefined;
  return (
    <button
